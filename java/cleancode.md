...
* if,else,while等语句,其中的代码块应该只有一行,该行应该时一个函数语句的调用
...
## 使用描述性的名称
* 长而具有描述性的名称,要比短而令人费解的名称要好;长而具有描述性的名称,要不描述性的注释要好

## 函数参数
* 最理想的参数量时零个,其次时一个,再次时两个,应尽量避免三个参数
...
### 参数对象
* 如果函数需要连个,三个或三个以上的参数,就说明其中一些参数应该封装为类
...
### 抽离try/catch代码块
* try/catch搞乱了代码的结构,把错误处理于正常流程混为一谈
...
## 小结
* 程序员把系统当做故事来讲,而不是程序来写,真正的目标自傲与讲述系统的故事,编写的函数必须干净利落的拼装在一起,形成一种精确而清晰的语言,帮助你讲故事
