...
* 使用静态工厂方法而不是构造器的优势
	...
	3. 返回原返回类型的任意子类型对象,返回类型可以是私有类型,目的是隐藏实现的具体细节,这种方式适用于基于接口的框架
	4. 返回的对象类可以随着每次调用发生变化，取决于静态工厂方法的参数
	5. 方法返回的所属的类，在编写包含静态工厂方法可以不存在（JDBC）

* 静态工厂方法的缺点
	1. 如果类不含公有的或受保护的构造器,就不能子类化,无法使用继承的同时,鼓励使用组合的方式来代替继承
	2. 程序员很难发现，构造对象的静态工厂方法与其它静态方法实际上没有任何区别,只能通过惯用名称进行区分
		* from(T)，类型转换方法，返回T类型对应的实例
		* of(T...)，聚合方法，T多个参数，返回T的一个实例
		* valueOf(T)，比from、of更繁琐的方法，返回的实例与参数具有相同值，意义上是一种类型转换
		* instance(T)/getInstance(T)，返回的实例通过方法参数描述
		* create(T)/newInstance(T)，返回的实例每次都时新创建的
		* getType(T/newType(T)，Type表示工厂方法返回的类型
		* type(T)，getType、newType简化版
...
* Java语言规范不仅不会保证finalize()被及时的执行，而是根本不保证会它会被执行，不应该依赖于finalize()更新重要的持久状态
...
* 如果类具有自己特有的“逻辑相等”概念（不同于对象等同概念）,并且超类没有覆盖equals()以实现期望的行为，这时应该覆盖equals()（值类：Integer、Date，枚举是例外）

* 覆盖equals()约定
	* 自反性：对于任何非null的引用x，x.equals(x)必须返回true
	* 对称性：对于任何非null引用x、y，当x.equals(y)==true时，y.equals(x)也必须等于true
	* 传递性：对于任何非null的引用x、y、z，如果x.equals(y)==true，并且y.equals(z)，那么x.equals()必须也返回true
	* 一致性：对与任何非null的引用x、y，在x、y对象中信息没有被修改前提下，多次调用x.equals(y)必须一致的返回true或false
	* 对于任何非null引用x，x.equals(null)==false

* 我们无法在扩展一个父类时，在子类中增加新的字段，同时又保留equals约定的

* 无论类是否是不可变的，都不要使equals()依赖于不可靠的资源
...
* 协变返回类型作为泛型：覆盖方法的返回类型可以是被覆盖方法的返回类型的子类，有助于覆盖方法提供更多关于被返回对象的信息，并且在客户端不用进行转换

* 永远不要让客户端去做任何类库能够提客户完成的事情

* 线程安全的类实现Cloneable接口，clone()方法必须要同步

* Cloneable.clone()替代方案，提供一个拷贝构造器或者拷贝工厂
	//拷贝构造器包含一个参数
	public Yum(Yum yum);
	//拷贝工厂
